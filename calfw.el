...
arguments."
  (cl-loop for f in (cfw:component-selection-change-hooks component)
           do (condition-case err
                  (funcall f)
                (error (message "Calfw: Selection change / Hook error %S [%s]" f err)))))
...
(defun cfw:model-create-updated-view-data (model view-data)
  "[internal] Clear previous view model data from MODEL and return a new
model with VIEW-DATA."
  (append
...
(defun cfw:render-truncate (org limit-width &optional ellipsis)
  "[internal] Truncate a string ORG with LIMIT-WIDTH, like
...
WIDTH is width of the toolbar.  CURRENT-VIEW is a symbol of the current
view type. This symbol is used to select the button faces on the
toolbar.  PREV-CMD and NEXT-CMD are the moving view command, such as
...
(defun cfw:render-columns (day-columns param)
  "[internal] This function concatenates each rows on the days into a
string of a physical line.
...
width (default).
`cfw:render-line-breaker-wordwrap' breaks lines with
the emacs function `fill-region'.
...
(defun cfw:render-periods-days (date periods-stack cell-width)
  ...
  (when periods-stack
    (cl-loop for (begin end content) in periods-stack
             for beginp = (equal date begin)
             for endp = (equal date end)
...
(make-local-variable 'cfw:before-win-num)
(make-local-variable 'cfw:main-buf)
...
TEXT is a content to show."
  (let ((buf (get-buffer cfw:details-buffer-name))
    ...
    (unless (and buf (eq (buffer-local-value 'major-mode buf)
      ...
      (with-current-buffer buf
        (cfw:details-mode)
        (setq cfw:before-win-num before-win-num)))
    (with-current-buffer buf
      (let (buffer-read-only)
        (setq cfw:main-buf main-buf)
        (erase-buffer)
        (insert text)
        (goto-char (point-min))))
    (pop-to-buffer buf)))
...
The size of calendar is calculated from the window that shows BUFFER or
the selected window.  DATE is initial focus date. If it is nil, today is
selected initially.  BUFFER is the buffer to be rendered. If BUFFER is
nil, this function creates a new buffer named
`cfw:calendar-buffer-name'.  CUSTOM-MAP is the additional keymap that is
added to default keymap `cfw:calendar-mode-map'."
  (save-excursion
...
(cl-defun cfw:create-calendar-component-region
  ...
  "Insert markers of the rendering destination at current point and display
the calendar view.

This function returns a component object and stores it at the text
property `cfw:component'.

DATE is initial focus date. If it is nil, today is selected initially.
WIDTH and HEIGHT are reference size of the calendar view. If those are
nil, the size is calculated from the selected window.  KEYMAP is the
keymap that is put to the text property `keymap'. If KEYMAP is nil,
`cfw:calendar-mode-map' is used."
...
(cl-defun cfw:get-calendar-text
    (width height &key date ;; keymap
           contents-sources annotation-sources view sorter)
