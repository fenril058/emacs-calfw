...
(defun cfw:ical-event-get-dates (event zone-map)
...
(defun cfw:ical-convert-event (event zone-map)
  (cl-destructuring-bind (dtag date start end) (cfw:ical-event-get-dates event zone-map)
    (make-cfw:event
     :start-date  date
     :start-time  start
...
(defun cfw:ical-convert-ical-to-calfw (ical-list)
  (cl-loop with zone-map = (icalendar--convert-all-timezones ical-list)
           ...
           for event = (cfw:ical-convert-event e zone-map)
           if event
...
;; (cfw:ical-data-cache-clear-all)
;; (progn (eval-buffer) (cfw:open-ical-calendar "http://www.cantonbecker.com/astronomy-calendar/astrocal.ics"))
;; (progn (eval-buffer) (cfw:ical-debug "http://www.cantonbecker.com/astronomy-calendar/astrocal.ics"))
