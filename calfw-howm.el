...
(defvar cfw:howm-schedule-summary-transformer
  (lambda (line) line)
  "Transformation function which transforms the howm summary string to
calendar title.  If this function splits into a list of string, the
calfw displays those string in multi-lines.")

(defun cfw:howm-schedule-parse-line (line)
  "[internal] Parse the given string and return a result list,
(date num type summary)."
...
from the howm schedule data."
  (cl-loop with contents = nil
           ...
           for (_ num type summary) = (cfw:howm-schedule-parse-line (howm-item-summary i))
           for transformed-summary = (funcall cfw:howm-schedule-summary-transformer summary)
           do
           (cond
            ((and (string= type "@") (< 0 num))
             (push (list date (cfw:date-after date (1- num)) transformed-summary) periods))
            ((and (string= type "!") (< 0 num))
             (push (list (cfw:date-before date (1- num)) date transformed-summary) periods))
            (t
...
`cfw:howm-from-calendar'.  This command should be executed on the calfw
calendar."
  (interactive)
  (let* ((mdy (cfw:cursor-to-nearest-date))
         (m (calendar-extract-month mdy))
...
(defun cfw:howm-schedule-inline (&optional width height view)
  "Inline function for the howm menu. See the comment text on the top of
this file for the usage."
  (let* ((custom-map (copy-keymap cfw:howm-schedule-inline-keymap))
         (cp (cfw:create-calendar-component-region
...
(declare-function elscreen-one-screen-p "elscreen")
(declare-function elscreen-create "elscreen")

(with-eval-after-load 'elscreen-howm
